...
import com.example.demo.service.export.ExportClientExel;
...
public class ExportClientController {
    ...
    @Autowired
    private ExportClientExel exportClientExel;

    /**
     * Export des clients au format CSV, déclenché sur l'url http://.../export/client/csv
     *
    ...
    public void clientCSV(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        response.setHeader("Content-Disposition", "attachment; filename=\"clients-csv-avec-age.csv\"");
    ...
    /**
     * Export des clients au format xlsx, déclenché sur l'url http://.../export/client/xlsx
     *
     * @param request  objet reprensantant la requête http
     * @param response objet reprensantant la réponse http
     */
    ...
    public void clientXlsx(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        exportClientExel.exportAll(outputStream);
    }

    @GetMapping("/clients/{clientId}/factures/xlsx")
    public void clientWithInvoiceXlsx(HttpServletRequest request, HttpServletResponse response, @PathVariable Long clientId) throws IOException {

        response.setContentType("application/vnd.ms-exel");
        response.setHeader("Content-Disposition", "attachment; filename=\"export-client-factures.xlsx\"");

        OutputStream outputStream = response.getOutputStream();

        exportClientExel.exportOneWithInvoice(outputStream, clientId);
    }
